...
if (source.includes("CRM_PRE_SALE_URL") && source.includes("fetch(CRM_PRE_SALE_URL")) {
  console.log("A integração do site principal com o WebTurbo CRM já está aplicada.");
  process.exit(0);
}
...
replaceOnce(
  "mensagem HTTP do CRM",
  '            erro: data.message || data.motivo || "erro_envio"',
  '            erro: data.message || data.error?.message || data.motivo || "erro_envio"'
);
replaceOnce(
  "mensagem visível do CRM",
  '          erroEl.textContent = data.message || "Erro ao enviar. Tente novamente.";',
  '          erroEl.textContent = data.message || data.error?.message || "Erro ao enviar. Tente novamente.";'
);
replaceOnce(
  "mensagem de conexão",
...
if (!source.includes("fetch(CRM_PRE_SALE_URL")) {
  throw new Error("O envio final não foi direcionado ao CRM.");
}
if (!source.includes('const CLOUD_RUN_URL = "https://modal-easy-')) {
  throw new Error("O endpoint auxiliar antigo foi removido indevidamente.");
}
if (!source.includes('action: "notifyConsulta"')) {
  throw new Error("A notificação auxiliar de consulta não foi preservada.");
}
if (!source.includes("TELEGRAM_ABANDONO_ENDPOINT = CLOUD_RUN_URL")) {
  throw new Error("A recuperação de abandono não foi preservada.");
}

writeFileSync(file, source, "utf8");
console.log("Patch aplicado em index.html.");
console.log("Somente o envio final da pré-venda foi movido para o WebTurbo CRM.");
console.log("Consulta de cobertura, notifyConsulta, recuperação de abandono, GA4/Google Ads e redirecionamento ao WhatsApp foram preservados.");
