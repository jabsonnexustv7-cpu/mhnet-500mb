...
(function () {
  ...
  function buildRecoveryPayload() {
    ...
    return {
      ...
      payload: {
        ...
        etapa_abandono: "whatsapp_principal_concluido",
        etapaAbandono: "whatsapp_principal_concluido",
        etapa: "whatsapp_principal_concluido",
        ...
        emailCliente: email,
        telefone1: phone,
        telefone1Cliente: phone,
        telefone2: digits(byId("mTelefone2")?.value),
        ...
        ponto_referencia: clean(byId("mPontoRef")?.value),
        vencimento: clean(byId("mVencimento")?.value),
        coordenadas: clean(byId("mCoordenadasFixas")?.value),
        link_localizacao: clean(byId("mLinkLocalizacaoFixa")?.value),
  ...
  async function sendRecoveryNow(reason) {
    ...
    try {
      const response = await window.fetch(TELEGRAM_ENDPOINT, {
        method: "POST",
      ...
      if (!response.ok || data?.ok === false || data?.telegramSent === false) {
        ...
        throw new Error(apiErrorMessage(data, response.status) || "Falha ao enviar recuperação");
      }
  ...
  // Trava final contra duplicatas da recuperação, inclusive das rotinas legadas.
    ...
    try {
      ...
      if (url === TELEGRAM_ENDPOINT && typeof init?.body === "string") {
        ...
        if (payload?.action === "notifyAbandonoModal") {
          ...
          if (key && telegramSent.has(key)) {
            return new Response(JSON.stringify({ ok: true, skipped: true, telegramSent: true, reason: "duplicate_same_visit" }), {
              status: 200,
              headers: { "Content-Type": "application/json" }
            });
          }
          ...
          const data = await response.clone().json().catch(() => ({}));
          if (!response.ok || data?.ok === false || data?.telegramSent === false) {
            if (key) telegramSent.delete(key);
          }
          return response;
        }
      }
  ...
  async function submitOrder(button) {
    ...
    try {
      ...
      if (!response.ok || data?.ok !== true || !data?.preSaleId) {
        const reason = apiErrorMessage(data, response.status) || "O CRM recusou o envio.";
        throw new Error(reason);
      }

      track("checkout_crm_direto_sucesso", {
        pre_sale_id: String(data.preSaleId),
        created: data.created === true ? "sim" : "nao"
      });
      submitting = false;
      if (button) {
        button.disabled = true;
        button.textContent = "Pedido concluído";
      }
  ...
  // Backup para qualquer chamada legada remanescente.
  ...
  document.addEventListener("input", function (event) {
    ...
    if (digits(event.target.value).length >= 10) {
      window.setTimeout(() => void sendRecoveryNow("whatsapp_completo"), 0);
    }
  }, true);
  document.addEventListener("blur", function (event) {
    if (event.target?.id === "mTelefone1" && digits(event.target.value).length >= 10) {
      void sendRecoveryNow("whatsapp_blur");
    }
  }, true);
  ...
  window.setInterval(() => {
    if (visibleStepFour()) bindStepFourButton();
  }, 1500);
})();
