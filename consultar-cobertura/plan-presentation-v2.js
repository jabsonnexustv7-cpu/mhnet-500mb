...
(function () {
  ...
  function cardFor(plan) {
    return document.querySelector(`#metaPlanGrid .meta-plan-card[data-plan="${CSS.escape(plan)}"]`);
  }
  ...
  function injectStyles() {
    ...
    style.textContent = `
      #metaPlanGrid .meta-plan-card.wt-plan-recommended{
        border-color:#00c853;
        background:linear-gradient(180deg,#f4fff8,#fff 46%);
        box-shadow:0 12px 30px rgba(0,200,83,.14)
      }
      #metaPlanGrid .meta-plan-card.wt-plan-recommended.is-selected{
        border-color:#1a56db;
        background:#f4f8ff;
      ...
      @media(max-width:600px){
        #metaPlanGrid{grid-template-columns:1fr!important}
        #metaPlanGrid .meta-plan-card{min-height:0}
        #metaPlanGrid .meta-plan-card-description{min-height:0}
      }
    `;
  ...
  document.addEventListener("change", function (event) {
    if (event.target?.id === "mPlano") {
      requestAnimationFrame(() => {
        updateHeader();
        track("plano_principal_selecionado", {
